...
@CompileStatic
class ApiGatewayGetProductRequestHandler implements ApiGatewayRequestHandler {
  ...
  APIGatewayV2HTTPResponse handleRequest(APIGatewayV2HTTPEvent event, Context context) {
    ...
    if (product.isEmpty()) {
      ...
      return prepareResponse(NOT_FOUND, '{"message": "Product not found"}')
    }

    logger.info product.toString()

    try {
      return prepareResponse(OK, toJson(product.get()))
